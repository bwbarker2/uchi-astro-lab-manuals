...
\begin{steps}
	\item Identify an object to be your ``nearby star'' and another as your ``distant star'', by analogy to the figure in the worksheet. Find a place where you can stand and move a meter or two side to side and still see both ``stars''. The movement will simulate the Earth moving from its January to its July position.
...
\begin{framed}
	The equations we will use later require the following. Make sure these are true. If not, choose different stars.
	\begin{itemize}
		\item The background star is at least 10 times as far away as the nearby star.
		\item The distance to the nearby star is at least 10 times the distance between your July and January Earth positions.
	\end{itemize}
\end{framed}
...
\begin{steps}
	\item Take a picture from a digital camera (likely your phone camera) from the vantage point of each Earth position used above. See Figure~\ref{par:fig:images} for example data.
...
\begin{figure}
	\includegraphics[width=0.5\textwidth]{parallax-groupform/parallax-image-1}
	\includegraphics[width=0.5\textwidth]{parallax-groupform/parallax-image-2}
...
You will use an image analysis program to measure lengths in pixels and find the pixel scale. These instructions are for using ImageJ, a free downloadable software tool, but you can use your own if you have a different preference.
